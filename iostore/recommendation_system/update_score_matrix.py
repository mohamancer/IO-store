...
def init_matrix():
    ...
    matrix = np.zeros((offers_amount,users_amount))
    bid_list = Bid.objects.all()

    for bid in bid_list:
        bidder_id = bid.bidder.id - 1
        offer_id = bid.offer.id - 1
        if (offer_id,bidder_id) not in s:
            matrix[offer_id][bidder_id] += 2.0
            s.add((offer_id,bidder_id))
    matrix_df = pd.DataFrame(matrix)
    return matrix_df

def add_user_columns_to_matrix_df(new_users):
    global matrix_df
    for new_user in new_users:
        matrix_df[len(matrix_df.columns)] = 0.0

def add_offer_rows_to_matrix_df(new_offers):
    global matrix_df
...
def add_bids(new_bids):
    for bid in new_bids:
        ...
        offer_id = bid.offer.id - 1
        if (offer_id,bidder_id) not in s:
            matrix_df[offer_id][bidder_id] += 2.0
            s.add((offer_id,bidder_id))
...
def update_matrix():
    ...
    while True:
        ...
        add_user_columns_to_matrix_df(new_users)
        add_offer_rows_to_matrix_df(new_offers)
        add_bids(new_bids)
