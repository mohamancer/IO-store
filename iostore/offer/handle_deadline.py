...
def mark_offers_inactive_by_deadline():
    try:
        make_inactive = Offer.objects.filter(
            bidding_deadline__lte=timezone.now(), active=True)
...
def periodically_mark_offers_inactive_by_deadline_thread():
    update_thread = threading.Thread(
        target=periodically_mark_offers_inactive_by_deadline, name='update_thread', daemon=True)
    update_thread.start()
